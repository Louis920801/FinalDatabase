...
def select_with_headers(mysql, table_name, limit=100):
    ...
    # 直接查資料；欄名用 cursor.description 拿
    cur.execute(f"SELECT * FROM `{table_name}` LIMIT %s", (limit,))
    rows = cur.fetchall()
    desc = cur.description or []                   # 可能 None
    headers = [d[0] for d in desc]                 # ['col1','col2',...]

    # 正規化：不論 rows 是 tuple 還是 dict，都轉成依 headers 排序的 list
    out = [headers]
    if rows:
        first = rows[0]
        if isinstance(first, dict):                # DictCursor
            for r in rows:
                out.append([r.get(h) for h in headers])
        else:                                      # tuple/list rows
            for r in rows:
                out.append(list(r))
    cur.close()
    return out
